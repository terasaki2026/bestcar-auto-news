...
# 初回起動時などでデータがない場合の自動取得（無限ループ防止）
if not news_items:
    # セッション内でリトライ回数を制限（最大2回まで）
    if "initial_fetch_tried" not in st.session_state:
        st.session_state["initial_fetch_tried"] = 0
    
    if st.session_state["initial_fetch_tried"] < 2:
        st.session_state["initial_fetch_tried"] += 1
        if refresh_news():
            st.rerun()
        else:
            st.info("⏳ ニュースの自動取得に失敗しました。サイドバーの「最新ニュースに更新」ボタンを押してください。")
    else:
        st.info("⏳ まだニュースがありません。サイドバーの「最新ニュースに更新」ボタンを押してニュースを取得してください。")
...
if st.sidebar.button("🔄 最新ニュースに更新", use_container_width=True):
    st.session_state["initial_fetch_tried"] = 0  # リトライカウンターをリセット
    if refresh_news():
